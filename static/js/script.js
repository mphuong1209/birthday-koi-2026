...
if (typewriterMessage) {
  ...
  // Click vào khung lời chúc để hiện ngay toàn bộ nội dung mà không cần chờ gõ
  const giftCard = typewriterMessage.closest(".gift-message-card");
  if (giftCard) {
    giftCard.style.cursor = "pointer";
    giftCard.addEventListener("click", () => {
      isFinished = true;
      isTypingTitle = false;
      twTitle.textContent = titleStr;
      twBody.textContent = bodyStr;
    });
  }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const PAGE_CONFIGS = [
    ...
    { page: 10, count: 4, title: 'Ảnh tốt nghiệp của chúng mình', isSpread: false }
  ];
